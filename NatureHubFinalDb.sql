...
-- Users Table
ALTER TABLE Users ALTER COLUMN Email NVARCHAR(60)
ALTER TABLE Users
ADD CreatedDate DATETIME NOT NULL DEFAULT GETDATE();
ALTER TABLE Users
DROP CONSTRAINT CHK_Password_Validation_User;
ALTER TABLE Users
ADD CONSTRAINT CHK_Password_Validation_Users CHECK (
    LEN(Password) >= 8 AND
    Password LIKE '%[A-Z]%' AND  -- At least one uppercase letter
    Password LIKE '%[a-z]%' AND  -- At least one lowercase letter
    Password LIKE '%[0-9]%' AND  -- At least one digit
    Password LIKE '%[!@#$%^&*(),.?":{}|<>]%' -- At least one special character
);
CREATE TABLE Users (
...
insert into Cart values(10,40,5)
select * from Cart
